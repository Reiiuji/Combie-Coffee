...
export default function LaporanPage() {
  ...
  // Panggil fetchLaporan saat halaman pertama kali dibuka
  useEffect(() => {
    fetchLaporan();
    ...
    <div className="min-h-screen bg-[#F9FAFB] font-sans pb-10">
      ...
      <style jsx global>{`
        @media print {
          ...
          /* Pastikan warna background tabel tercetak */
          th { background-color: #E6A05B !important; color: white !important; }
          tr:nth-child(even) { background-color: #F3F4F6 !important; }
        }
      `}</style>

      {/* HEADER ORANGE */}
      <header className="bg-[#E6A05B] px-8 py-5 text-white shadow-sm flex justify-between items-center mb-8 no-print">
      ...
      <main className="px-8 max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-8 no-print">
            <h1 className="text-xl font-bold text-gray-800 mb-6 pb-2 border-b border-gray-100 flex items-center gap-2">
                <Filter size={20} className="text-[#E6A05B]" />
            ...
            <div className="flex flex-col md:flex-row gap-6 items-end">
                <div className="w-full md:w-1/3">
                    ...
                    <input 
                        ...
                        className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-[#E6A05B] outline-none transition-all"
                    />
                </div>
                ...
                <div className="w-full md:w-auto flex gap-3">
                    <button 
                        ...
                        disabled={loading}
                        className="bg-[#E6A05B] hover:bg-[#d48e4d] text-white px-6 py-3 rounded-lg font-bold shadow-sm flex items-center justify-center gap-2 disabled:opacity-50 transition-all active:scale-95"
                    >
                        {loading ? 'Memuat...' : (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden print-container">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="bg-[#E6A05B] text-white">
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider text-center border-r border-orange-300/30">No</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider border-r border-orange-300/30">Tanggal & Waktu</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider border-r border-orange-300/30">Nama Pelanggan</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider border-r border-orange-300/30">Menu/Item</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider text-center border-r border-orange-300/30">QTY</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider text-right border-r border-orange-300/30">Harga/Item</th>
                            <th className="px-6 py-4 text-sm font-bold uppercase tracking-wider text-right border-r border-orange-300/30">Total Bayar</th>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {laporan.length === 0 ? (
                            ...
                            laporan.map((row, index) => (
                                <tr key={index} className="even:bg-gray-50 hover:bg-orange-50/50 transition-colors">
